...
class SettingsScreen(BaseScreen):
    ...
    def save_settings(self) -> bool:
        ...
        Returns True if successful, False on validation error or crash.
        """
        try:
            ...
            if min_delay < 0 or max_delay < 0 or log_limit < 1 or polling_delay < 0:
                self.app.notify("Values must be positive!", severity="error")
                return False

            if max_delay < min_delay:
                self.app.notify("Max delay must be >= Min delay!", severity="error")
                return False
            ...
            if old_lang != self.selected_lang:
                ...
                try:
                    ...
                    # Fallback to execv if Popen fails (unlikely)
            ...
            self.app.notify(f"Settings Save Error: {e}", severity="error")
            return False
